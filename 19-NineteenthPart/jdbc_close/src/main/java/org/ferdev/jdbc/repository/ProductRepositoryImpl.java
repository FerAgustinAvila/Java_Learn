...
public class ProductRepositoryImpl implements Repository<Product>{
    ...
    public Product byId(Long id) {
        ...
        try(
                Connection conn = getConnection();
                PreparedStatement stmt = conn.prepareStatement("SELECT p.*, c.nombre as categoria FROM " +
                        "productos as p inner join categorias as c ON (p.categoria_id = c.id) WHERE p.id = ?");
            ) {
            stmt.setLong(1, id);
